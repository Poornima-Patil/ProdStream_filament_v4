    ...
    public function infolist(Schema $schema): Schema
    {
        return $schema->components([
            ...
            Section::make('Work Order Analytics & Distribution')
                ->collapsible()
                ->columnSpanFull()
                ->description('Filter work orders and view analytics for this operator proficiency level')
                ->headerActions([
                ...
                ->schema([
                    ...
                    TextEntry::make('work_order_analytics')
                        ...
                        ->getStateUsing(function ($record) {
                            if (! $record) {
                                return new \Illuminate\Support\HtmlString('<div class="text-gray-500 dark:text-gray-400">No Operator Proficiency Found</div>');
                            }

                            // Get work order status distribution for operators with this proficiency level
                            // We need to join with operators table since operator proficiencies are related through operators
                            $summaryQuery = \Illuminate\Support\Facades\DB::table('work_orders')
                                ->join('operators', 'work_orders.operator_id', '=', 'operators.id')
                                ->where('operators.operator_proficiency_id', $record->id)
                    ...
                    Livewire::make(OperatorProficiencyStatusChart::class, [
                        'record' => $this->record,
