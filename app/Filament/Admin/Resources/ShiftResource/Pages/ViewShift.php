    ...
    public function infolist(Schema $schema): Schema
    {
        return $schema->components([
            ...
            Section::make('Work Order Analytics & Distribution')
                ->collapsible()
                ->columnSpanFull()
                ->description('Filter work orders and view analytics for this shift')
                ->headerActions([
                ...
                ->schema([
                    ...
                    TextEntry::make('work_order_analytics')
                        ...
                        ->getStateUsing(function ($record) {
                            if (! $record) {
                                return new \Illuminate\Support\HtmlString('<div class="text-gray-500 dark:text-gray-400">No Shift Found</div>');
                            }

                            // Get work order status distribution for shift operators
                            // We need to join with operators table since shifts are related through operators
                            $summaryQuery = \Illuminate\Support\Facades\DB::table('work_orders')
                                ->join('operators', 'work_orders.operator_id', '=', 'operators.id')
                                ->where('operators.shift_id', $record->id)
                    ...
                    Livewire::make(ShiftStatusChart::class, [
                        'record' => $this->record,
