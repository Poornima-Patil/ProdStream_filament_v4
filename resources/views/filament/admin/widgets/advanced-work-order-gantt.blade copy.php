                                ...
                                <tr class="relative" style="height: 64px;">
                                    ...
                                    @php
                                        ...
                                        foreach ($workOrders as $wo) {
                                            ...
                                            $actualEndLog = $wo->workOrderLogs->whereIn('status', ['Closed', 'Completed'])->sortByDesc('changed_at')->first();
                                        ...
                                        <td class="align-top border relative" style="height: {{ $cellHeight }}px;">
                                            ...
                                            @foreach($cellBars as $bar)
                                                ...
                                                @elseif($bar['type'] === 'actual')
                                                    <a href="{{ url('admin/' . $factoryId . '/work-orders/' . $wo->id) }}"
                                                       class="absolute rounded flex items-center overflow-hidden group"
                                                       style="top: {{ $barTop + $stackIdx * 2 * ($barHeight + $barGap) + $barHeight + $barGap }}px; left: 0; height: {{ $barHeight }}px; width: calc({{ $spanDays }} * 100% + ({{ $spanDays-1 }} * 2px)); z-index: 20; text-decoration: none;"
                                                       title="{{ $wo->unique_id }}">
                                                    @if($okPercentage > 0)
                                                        <div class="h-full flex items-center" style="background: {{ $okColor }}; width: {{ $okPercentage }}%;">
                                                            <span class="text-xs text-white font-semibold px-2 truncate w-full" style="line-height: {{ $barHeight }}px;">
                                                                {{ round($okPercentage) }}%
                                                            </span>
                                                        </div>
                                                    @endif
                                                    @if($scrappedPercentage > 0)
                                                        <div class="h-full bg-red-500 flex items-center" style="width: {{ $scrappedPercentage }}%;">
                                                            <span class="text-xs text-white font-semibold px-2 truncate w-full" style="line-height: {{ $barHeight }}px;">
                                                                {{ round($scrappedPercentage) }}%
                                                            </span>
                                                        </div>
                                                    @endif
                                                    </a>
                                    ...
                                    <tr>
                                        ...
                                        @for($h = 0; $h < 24; $h++)
                                            @php
                                                ...
                                                $factoryId = auth()->user()?->factory_id ?? 'default-factory';
                                            @endphp
                                            <td class="relative border align-top p-0" style="height: 36px;">
                                                @if($h === $startHour)
                                                    <a href="{{ url('admin/' . $factoryId . '/work-orders/' . $wo->id) }}"
